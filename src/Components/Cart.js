...
const Cart = ()=> {
  ...
  <div>
    ...
    {prod.product.name} 
    <Button onClick={() => _removeFromCart(prod.product, prod.quantity)}>X</Button> 
  </div>
</Typography>
        ...
        <Card sx={{ minWidth: 275, maxWidth: 800, textAlign:"right" }}>
          ...
          <CardActions  sx={{justifyContent:"flex-end"}}>
            <Button component='span' size="small" sx={{fontSize: 18}}>
              {getTotal() === 0 ? <Link to='/menu' sx={{color: "black"}}>Checkout Your Cart</Link> : <Link to='/checkout'>Checkout</Link>}</Button>
          </CardActions>
        </Card>
      </div>
    </div>
